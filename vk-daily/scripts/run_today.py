...
def main() -> int:
    ...
    ap.add_argument("--headline", default="", help="Specific headline to use")
    ap.add_argument("--composition-id", default="", help="Specific composition ID to use")
    ap.add_argument("--post-file", default="", help="Path to custom post file to use as-is")
    args = ap.parse_args()
    ...
    if args.post_file:
        custom_post = Path(args.post_file)
        if not custom_post.is_absolute():
            custom_post = root / custom_post
        if custom_post.is_file():
            artifact["post"] = custom_post.read_text(encoding="utf-8").strip()
            artifact["char_count"] = len(artifact["post"])
    if args.headline:
    ...
    meta = {
        ...
        "office_ru": tenant.get("office_ru") or "ул. Жукова 39/1, офис 303, Уфа (Сипайлово)",
