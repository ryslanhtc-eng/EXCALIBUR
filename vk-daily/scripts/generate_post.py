...
def _headlines_for(news: dict) -> list[str]:
    ...
    if angle == "mortgage_window":
        return [
            "Семейная ипотека к 1 октября",
            "Семейная ипотека к первому октября",
...
def pick_headline(news: dict, seed: str, tenant: dict) -> str:
    if news.get("id") == "family_mortgage_oct":
        return "Семейная ипотека к 1 октября"
...
def _fit_length(text: str, min_c: int, max_c: int) -> str:
    body = text.strip()
    filler = (
        "\n\nЕщё одна практическая проверка перед показом: посмотрите дом в обычный будний час, "
