...
def _headlines_for(news: dict) -> list[str]:
    ...
    if angle == "kids_share":
        return [
            "Детская доля и риск отмены",
            "Одного согласия опеки мало",
            "Как не потерять квартиру детям",
            "Вторичка опека и встречная покупка",
        ]
...
def _kids_share_body(_news: dict) -> str:
...
def render_post(news: dict, tenant: dict) -> str:
    ...
    elif angle == "kids_share":
        raw = _kids_share_body(news)
    else:
