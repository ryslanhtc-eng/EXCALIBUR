...
def _headlines_for(news: dict) -> list[str]:
    ...
    if angle == "parents_gift_protect":
        return [
            "Деньги родителей на квартиру при разводе",
        ]
...
def _parents_gift_body(_news: dict) -> str:
    return """Знакомая ситуация для сотен уфимских семей этой осенью. Молодая пара покупает квартиру в Сипайлово или Зелёной Роще, а родители с одной стороны вносят два-три миллиона рублей на первый взнос. От чистого сердца, наличными из серванта или простым переводом на карту со словами "детям на счастье".
